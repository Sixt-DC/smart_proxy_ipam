module NetboxHelper
  def validate_required_params(required_params, params)
    ...
    required_params.each do |param|
      err.push(errors[param.to_sym]) unless params[param.to_sym]
    end
    err.empty? ? [] : { code: 400, error: err }.to_json
  end
  ...
  def filter_fields(json_body, fields)
    data = []
    if json_body && json_body['data']
      json_body['data'].each do |subnet|
        item = {}
        fields.each { |field| item[field.to_sym] = subnet[field.to_s] }
        data.push(item)
      end
    end
    data
